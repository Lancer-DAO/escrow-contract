...
import { COMPLETER_FEE, LANCER_FEE, MONO_LOCALNET, WSOL_ADDRESS } from "../../sdk/constants";
...
describe("fund feature tests", () => {
    ...
    const program = new Program<MonoProgram>(
      ...
      it ("test achfundFeature Works", async () => {
        let creator = await createKeypair(provider);
        let external_funder = await createKeypair(provider);
    
        const external_funder_wsol_account = await getOrCreateAssociatedTokenAccount(
            provider.connection,
            external_funder,
            WSOL_ADDRESS,
            external_funder.publicKey
        );
    
        await add_more_token(provider, external_funder_wsol_account.address, WSOL_AMOUNT);
        ...
        // test insuffiecient 
        try {
          await program.methods.achFundFeature(new anchor.BN(WSOL_AMOUNT))
            .accounts({
              creator: creator.publicKey,
              fundsMint: WSOL_ADDRESS,
              externalFunder: external_funder.publicKey,
              externalFunderTokenAccount: external_funder_wsol_account.address,
              featureDataAccount: feature_data_account,
              featureTokenAccount: feature_token_account,
              programAuthority: program_authority,
              tokenProgram: TOKEN_PROGRAM_ID,
              systemProgram: SystemProgram.programId,
            }).signers([external_funder]).rpc()
        } catch (err) { 
          assert.equal((err as AnchorError).error.errorMessage, "Insufficient funds to pay lancer fee")
        }
    
        //add more SOL
        await add_more_token(provider, external_funder_wsol_account.address, WSOL_AMOUNT * 5 / 100);
    
        // check balaance before funding feature
        const FFA_token_account_before_balance = await provider.connection.getTokenAccountBalance(feature_token_account)
        let ach_fund_feature_ix = await achFundFeatureInstruction(
          WSOL_AMOUNT,
          acc.unixTimestamp,
          external_funder.publicKey,
          creator.publicKey,
          ...
          const tx2 = await provider.sendAndConfirm(new Transaction().add(ach_fund_feature_ix), [external_funder]);
          console.log("achFundFeature transaction signature", tx2);
          const FFA_token_account_after_balance = await provider.connection.getTokenAccountBalance(feature_token_account)
          assert.equal(
            FFA_token_account_after_balance.value.amount, 
            (//token account needs to be able to pay both lancer and completer
      ...
      // TODO - Add to tests.yaml
      it ("if admin = creator, external funder paying for creator(achfundfeature) does not pay extra 5% upfront", async () => {
        let payer = await createKeypair(provider);
        let external_funder = await createKeypair(provider);
        let funder = await provider.publicKey;
      ...
      const [program_authority] = await findProgramAuthority(
        ...
        let bal = await provider.connection.getTokenAccountBalance(external_funder_wsol_account.address);
        let amount = bal.value.uiAmount * LAMPORTS_PER_SOL;
        // test insuffiecient 
        try {
          await program.methods.achFundFeature(new anchor.BN(amount + 1))
            .accounts({
              creator: funder,
              fundsMint: WSOL_ADDRESS,
              externalFunder: external_funder.publicKey,
        ...
        let fund_feature_ix = await achFundFeatureInstruction(
          ...
          const tx2 = await provider.sendAndConfirm(new Transaction().add(fund_feature_ix), [external_funder]);
          console.log("fundFeature transaction signature", tx2);
          const FFA_token_account_after_balance = await provider.connection.getTokenAccountBalance(feature_token_account)
          assert.equal(
            FFA_token_account_after_balance.value.amount, 
      ...
      // TODO - Add to tests.yaml
      it ("test amount gets added on if achFundFeature is called twice", async () => {
        let creator = await createKeypair(provider);
        ...
        // check balaance before funding feature
        const FFA_token_account_before_balance = await provider.connection.getTokenAccountBalance(feature_token_account)
        let ach_fund_feature_ix1 = await achFundFeatureInstruction(
          WSOL_AMOUNT,
          acc.unixTimestamp,
          external_funder.publicKey,
          creator.publicKey,
        ...
        let ach_fund_feature_ix2 = await achFundFeatureInstruction(
          WSOL_AMOUNT,
          acc.unixTimestamp,
          external_funder.publicKey,
          creator.publicKey,
        ...
        const tx2 = await provider.sendAndConfirm(
          new Transaction()
            .add(ach_fund_feature_ix1)
            .add(ach_fund_feature_ix2),
            [external_funder]
        );
      ...
      // TODO Add to tests.yaml
      it ("(admin = creator)Test amount gets added but still no lancer fee if achFundFeature is called twice", async () => {
        let payer = await createKeypair(provider);
        let external_funder = await createKeypair(provider);
        let funder = await provider.publicKey;
      ...
      // transfer any funds in funder wallet so we are certain funder wallet is empty before tests
        let external_funder_current_wsol_amount = await provider.connection.getTokenAccountBalance(
          external_funder_wsol_account.address          
        )
        ...
        let transfer_ix = await createTransferInstruction(
          ...
          payer_wsol_account.address,
          external_funder.publicKey,
          parseInt(external_funder_current_wsol_amount.value.amount)
        )

        await provider.sendAndConfirm(new Transaction().add(transfer_ix), [external_funder]);
      ...
      const [program_authority] = await findProgramAuthority(
        ...
        // test insuffiecient acc
        try {
          await program.methods.achFundFeature(new anchor.BN(WSOL_AMOUNT + WSOL_AMOUNT))
            .accounts({
              creator: funder,
              fundsMint: WSOL_ADDRESS,
              externalFunder: external_funder.publicKey,
      ...
      it ("test full amount gets refunded when bounty is funded multiple times", async () => {
        let creator = await createKeypair(provider);
        ...
        //add more SOL
        await add_more_token(provider, creator_wsol_account.address, WSOL_AMOUNT * 2);
        // check balaance before funding feature
        let fund_feature_ix1 = await fundFeatureInstruction(
          WSOL_AMOUNT,
          acc.unixTimestamp,
        ...
        const tx2 = await provider.sendAndConfirm(new Transaction().add(fund_feature_ix1).add(fund_feature_ix2), [creator]);
        console.log("fundFeature transaction signature", tx2);
        const creator_token_account_before_balance = await provider.connection.getTokenAccountBalance(creator_wsol_account.address);
        acc = await program.account.featureDataAccount.fetch(accounts[0].pubkey);
        ...
        await provider.sendAndConfirm(new Transaction().add(vote_to_cancel_ix).add(cancel_ix), [creator]);

        const creator_token_account_after_balance = await provider.connection.getTokenAccountBalance(creator_wsol_account.address);

        assert.equal(
          parseInt(creator_token_account_after_balance.value.amount),
          parseInt(creator_token_account_before_balance.value.amount) + (acc.amount.toNumber() * (LANCER_FEE + COMPLETER_FEE))
        )

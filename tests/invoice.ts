...
import { ASSOCIATED_TOKEN_PROGRAM_ID, createMint, getAccount, getOrCreateAssociatedTokenAccount, mintToChecked, TOKEN_PROGRAM_ID } from "@solana/spl-token";
...
import { findFeatureAccount, findFeatureTokenAccount, findLancerProgramAuthority, findLancerTokenAccount, findProgramAuthority } from "../sdk/pda";
import { 
...
describe("invoice tests", () => {
  ...
  const program = new Program<MonoProgram>(
    ...
    it("test sending and accepting invoice works(when lancer admin does not accept invoice)", async () => {
        ...
        await add_more_token(provider, new_creator_wsol_account.address, WSOL_AMOUNT);
        ...
        const [new_feature_data_account, new_feature_data_account_bump] = await findFeatureAccount(
            timestamp,
            new_creator.publicKey,
            program
        );
        const [new_feature_token_account, new_feature_token_account_bump] = await findFeatureTokenAccount(
            timestamp,
            new_creator.publicKey,
            WSOL_ADDRESS,
            program
        );
        const [program_authority, program_authority_bump] = await findProgramAuthority(program);
        const ix = await program.methods.createFeatureFundingAccount(timestamp)
            .accounts({
        ...
        await provider.sendAndConfirm(new Transaction().add(sendInvoiceIx), [creator]);
        
        acc = await program.account.featureDataAccount.fetch(accounts[0].pubkey);

        assert.equal(acc.amount.toNumber(), WSOL_AMOUNT);
        assert.equal(acc.payoutAccount.toString(), new_creator.publicKey.toString())

        // test for previous creator cannot be new creator
        try {
            await program.methods.acceptInvoice()
                .accounts({
        ...
        try {
            await program.methods.acceptInvoice()
                .accounts({
                    ...
                    associatedProgram: ASSOCIATED_TOKEN_PROGRAM_ID,
                    systemProgram: SystemProgram.programId,
                }).signers([new_creator]).rpc();
        } catch (err) {
            assert.equal((err as AnchorError).error.errorMessage, "Insufficient funds to pay lancer fee")
            // add more funds
            await add_more_token(provider, new_creator_wsol_account.address, WSOL_AMOUNT / 2)
        }
        ...
        const new_creator_wsol_account_before_balance = await provider.connection.getTokenAccountBalance(new_creator_wsol_account.address);

        await provider.sendAndConfirm(new Transaction().add(acceptInvoiceIx), [new_creator]);
        console.log("accept invoice tx = ", tx)

        const FFA_token_account_after_balance = await provider.connection.getTokenAccountBalance(new_feature_token_account)
        const new_creator_wsol_account_after_balance = await provider.connection.getTokenAccountBalance(new_creator_wsol_account.address);


        assert.equal(
          FFA_token_account_after_balance.value.amount, 
          (//token account needs to be able to pay both lancer and completer
            ((LANCER_FEE + COMPLETER_FEE) * WSOL_AMOUNT) + parseInt(FFA_token_account_before_balance.toString())
          ).toString()
        );
        ...
        assert.equal(
          ...
          (//token account needs to be able to pay both lancer and completer
            ((LANCER_FEE + COMPLETER_FEE) * WSOL_AMOUNT) + parseInt(new_creator_wsol_account_after_balance.value.amount)
          ).toString()
        );
        ...
        acc = await program.account.featureDataAccount.fetch(new_feature_data_account);
        ...
        assert.equal(acc.fundsTokenAccount.toString(), new_feature_token_account.toString());
    ...
    it("test sending and accepting invoice works(when lancer admin does accepts invoice)", async () => {
        ...
        const special_mint = await createMint(
          ...
          const creator_special_mint_account = await getOrCreateAssociatedTokenAccount(
           ...
           const new_creator_special_mint_account = await getOrCreateAssociatedTokenAccount(
            provider.connection,
            mint_authority,
            special_mint,
            new_creator
        ...
        const FFA_token_account_before_balance = 0;
        const new_creator_special_mint_account_before_balance = await provider.connection.getTokenAccountBalance(new_creator_special_mint_account.address);

        await provider.sendAndConfirm(new Transaction().add(acceptInvoiceIx), []);
        console.log("accept invoice tx = ", tx)

        const FFA_token_account_after_balance = await provider.connection.getTokenAccountBalance(new_feature_token_account)
        const new_creator_special_mint_account_after_balance = await provider.connection.getTokenAccountBalance(new_creator_special_mint_account.address);
        ...
        assert.equal(
          ...
          (//token account needs to be able to pay both lancer and completer
            (WSOL_AMOUNT) + parseInt(new_creator_special_mint_account_after_balance.value.amount)
          ).toString()
        );
    ...
    it ("testing rejecting invoice", async () => {
        ...
        try {
            await program.methods.rejectInvoice()
                .accounts({
        ...
        await provider.sendAndConfirm(new Transaction().add(rejectInvoiceIx), [new_creator]);
        acc = await program.account.featureDataAccount.fetch(accounts[0].pubkey);
        ...
        assert.equal(acc.payoutAccount.toString(), another_invoice_acceptor.publicKey.toString());
    })    
    ...
    it ("sending invoice, rejecting and close invoice", async () => {
        ...
        await provider.sendAndConfirm(new Transaction().add(closeInvoiceIx), [creator]);
